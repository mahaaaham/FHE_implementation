...
$A$ est choisie en se basant sur le problème DLWE:
\begin{itemize}
\item pour de bons choix de paramètres, $A$ est indistinguable d'une matrice
choisie uniformément.
\end{itemize}

\textbf{Choix de C:}
Pour chiffrer $\mu \in \ZZq$:
\[ C :=\mu\:\text{Id} + A R \qquad R\in {\{0,1\}}^{m \times N}\: \text{ tirée uniformément.} \]

\textbf{Sécurité:} pour de bons choix de paramètres\\
...
Soit $a,b,c\in\{0, 1\}$ et $C_a, C_b, C_c$ leurs chiffrés pour des clés (sk, pk)
  \begin{center}
  ...
  \[ \Pi(a,c,b) = \text{Decrypt}\left(\text{sk}, \text{Eval}\left(\Pi, C_a, C_b, C_b\right)\right) \]
  On note alors:
...
\approx&\: \text{Eval}\left(\Pi, \text{Encrypt}(\text{sk}, \text{binsk})\right)
\end{align*}
\vspace{-0.5cm}
\begin{itemize}
