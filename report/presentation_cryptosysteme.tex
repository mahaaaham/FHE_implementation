\begin{section}{Présentation du cryptosystème}
	...
	\end{prop}
	\begin{proof}
	$\langle $\textbf{BitDecomp}$(\vec{a}),$ \textbf{PowersOf2}$(\vec{b}) \rangle = \sum\limits_{i,j} a_{i,j} \times (2^{j} \times b_i) = \sum\limits_{i,j} (a_{i,j} \times 2^{j}) \times b_i \sum\limits_{i} a_i \times b_i = \langle\vec{a},\vec{b} \rangle$.
	\end{proof}
