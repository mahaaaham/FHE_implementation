\begin{section}{Présentation du cryptosystème}
	...
	\end{subsection}
	\begin{subsection}{Définition du cyrptosystème}
	Nous allons ici définir le cryptosystème GSW, en utilisant 
	l'exposition originale faite dans \cite{EPRINT:GenSahWat13},
	légèrement modifiée pour prendre en compte des remarques interessantes
	faites par Shai Halevi dans \cite{halevi}.
	\paragraph{}
	\textbf{BitDecomp}
	\flushleft
	...
	\textbf{Sortie} : Cet algorithme retourne les paramètres $n, q, \chi,
	m$ du système. 
	Le paramètre $L$ indique que les paramètres crées doivent permettre d'évaluer un circuit de NAND de profondeur $L$ tout en pouvant
	déchiffrer correctement. Nous montrerons qu'il est alors nécessaire de
	satisfaire la condition \og dite de longueur \fg  
	\[q > 8nm (1 + N)^L \]

	\textbf{Algorithme}  
	Nous verrons dans la section des considérations sur les choix des
	paramètres.\\
	...
	\textbf{Sortie} : Cet algorithme la publique $A \in \ZZq^{m \times n}$
	ainsi que la clé secrète $\vec{s} \in \ZZq^{n+1}$ vérifiant la
	contrainte $A \cdot \vec{s} \leq n$. \\
	\textbf{Algorithme}: \\ 
	\textbf{clé secrète:}  On génère un vecteur $t \in \ZZq^n$ avec $\chi$
	et on définit la clé secrète comme $\text{\textbf{PowersOf2}}(\vec{s})$
	pour $\vec{s} = (1, -t_1, ..., -t_n)$. \\
	...
	Si la contrainte $A \cdot \vec{s} \leq n$ n'est pas vérifiée, on recrée un jeu de clés.
...
\begin{prop}
\label{dec}
\textbf{Dec} décrypte avec succès les chiffrés dont l'erreur $\vec{e}$
satisfait $\|\vec{e}\|_\infty < q/8$.
\end{prop}
\begin{proof}
	Dans ce cas, 
	On a $x_i = \mu * v_i + e$ avec $\lvert e \lvert \leq
	\|\vec{e}\|_\infty$ et comme 
	$\lvert v_i \lvert > \frac{q}{4}$, $\lvert \frac{e}{v_i} \lvert < 1/2$.
	On a donc $\lfloor \frac{x_i}{v_i} \rceil = \mu$.
\end{proof}
	\end{subsection}
