...
Comme la clé secrète est un vecteur de $\ZZq^N$ et qu'un élément de $\ZZq$ a une décomposition binaire d'au plus $\log(q)$ éléments, nous pouvons représenter la clé secrète par une liste de $N\log(q)$ 0 et 1.
...
seul les chiffrés des $N\log(q)$ éléments de la représentation en 0 et 1 de la clé secrète sera connu. 
...
\begin{proof}
	Il suffit de compléter les listes par des 0 pour se réduire au cas où la taille vaut $2^{\lceil \log(s) \rceil}$ et d'appliquer le travail fait précédemment.
\end{proof}
\end{subsubsection}
...
$8 \lceil \log(s) \rceil + 36\lceil\log_3(nb)\rceil -18$. Soit, une profondeur en $\mathcal{O}(\log(nb) + \log(s))$.
\end{prop}
...
utilisant aussi que $\log_3(n+1) \leqslant\log(n)$ (dès $n \geqslant 2)$.
\end{proof}
...
\begin{equation*}
n^{\rho \log^2(n)} > {\left(n\log(q))\right)}^{\:\rho\: \left(\log(n) + \log(\log(q)\right)}.
\end{equation*}
