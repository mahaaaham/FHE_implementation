...
\end{definition}
	...
	L'article \cite{cryptoeprint:2018:786} indique une borne interessante:
\begin{prop}
...
\begin{prop}
Pour $s \geq 1.5$ et $t > \sqrt(\log(n))$, si il existe une constante $\tau$
...
\end{prop}
\begin{proof}
On veut appliquer la proposition~\label{gaussienne}.
...
\begin{align*}
\PP\left(x\leftarrow D^q_s\: : \: |x|\geq q \alpha \right) =
\PP\left(x\leftarrow D^q_s\: : \: |x|\geq s \frac{q \alpha}{s}  \right)
\end{align*}
On en déduit qu'on peut appliquer la proposition précédente avec $t = q \alpha
/s$.
