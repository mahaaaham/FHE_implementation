...
\begin{itemize}
\item \path{GSW_scheme} contenant l'implémentation du cryptosystème GWS;
...
\begin{itemize}
\item \path{GSW_scheme.sage} contient les fonctions principales de GWS dont \path{setup}, \path{encrypt} et les 3
algorithmes de déchiffrements que sont \path{basic_decrypt} (l'algorithme \text{Decrypt} du rapport), \path{mp_decrypt}
...
chiffrer et déchiffrer les messages, comme par exemple \path{flatten}, ou encore une implémentation du nearest plane de
Babai;
...
Ce dossier contient les fichiers suivants:
\begin{itemize}
\item \path{depth_security.sage} contenant une fonction permettant de lancer \path{lwe_estimator};
 sur des paramètres définis dans \path{params_maker.sage} et une autre permettant de voir quel est le plus grand $L$
 satisfaisant la condition de longueur (équation \eqref{eq:longueur} page \pageref{eq:longueur}) sur des paramètres
 définis dans \path{params_maker.sage}
...
les lancer en  utilisant  la fonction \path{all_circuit_without_bs};
\item \path{all_circuit_analysis.sage} contient la fonction \path{analysis_main} qui lance les différents circuits avec
...
fonctions de test, puis un fichier de test correspondant à chaque fichier du dossier \path{GSW_scheme}.  Chacun de ces
